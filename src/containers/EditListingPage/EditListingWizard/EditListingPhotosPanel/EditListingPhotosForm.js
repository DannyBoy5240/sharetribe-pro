...
// ----- To make input element accept all kinds of files -----
const ACCEPT_EXTRA_FILES = '*/*';
...
// ----- Field component that uses file-input to allow user to select extra files. -----
export const FieldAddExtraFiles = props => {
  const { formApi, onExtraFilesUploadHandler, aspectWidth = 1, aspectHeight = 1, ...rest } = props;
  return (
    <Field form={null} {...rest}>
      {fieldprops => {
        const { accept, input, label, disabled: fieldDisabled } = fieldprops;
        ...
        const onChange = e => {
          ...
          formApi.change(`addImage`, file);
          formApi.blur(`addImage`);
          onExtraFilesUploadHandler(file);
        };
        const inputProps = { accept, id: name, name, onChange, type };
        return (
          <div className={css.addImageWrapper}>
...
export const EditListingPhotosFormComponent = props => {
  ...
  const [exState, setExState] = useState({ extraFilesUploadRequest: false });
  ...
  // ----- Extra Files Upload Handler Function -----
  const onExtraFilesUploadHandler = file => {
    console.log('onExtraFilesUploadHandler function called !');

    const { listingImageConfig, onImageUpload } = props;
    if (file) {
      setExState({ extraFilesUploadRequest: true });

      onImageUpload({ id: `${file.name}_${Date.now()}`, file }, listingImageConfig)
        .then(() => {
          setExState({ extraFilesUploadRequest: false });
        })
        .catch(() => {
          setExState({ extraFilesUploadRequest: false });
        });
      ...
      render={formRenderProps => {
        ...
        return (
          ...
          >
            ...
            {/* ----- Extra File Upload Function Part ----- */}
            <div className={css.imagesFieldArray}>
              <FieldAddExtraFiles
                id="addExtraFiles"
                name="addExtraFiles"
                accept={ACCEPT_EXTRA_FILES}
                label={
                  <span className={css.chooseImageText}>
                    <span className={css.chooseImage}>
                      <FormattedMessage id="EditListingPhotosForm.chooseExtraFiles" />
                    </span>
                    <span className={css.imageTypes}>
                      <FormattedMessage id="EditListingPhotosForm.extraFilesTypes" />
                    </span>
                  </span>
                }
                type="file"
                disabled={state.extraFilesUploadRequest}
                formApi={form}
                onExtraFilesUploadHandler={onExtraFilesUploadHandler}
                aspectWidth={aspectWidth}
                aspectHeight={aspectHeight}
              />
            </div>
